...
```
We won't be doing CV for SVM model, due to high similarity of the training, and test set accuracy (those models take a long time to evaluate, so we will skip this step).

```{r}
tic()
SVM_tuned <- tune.svm(RainTomorrow~., data=train,
                      degree = c(3,4),
                      gamma = c(.005 ,0.01 ,0.1),
                      cost = seq(0.1,0.9, by = 0.4),
                      epsilon = c(0.1,0.05)
                      )
toc()
